...
# tests Point_updates_are_not_reflected
def test_purchasePlaces_points_and_places_deduction(mocker):
    '''Test que les points et les places sont correctement déduits après une réservation réussie.'''
    initial_club_points = "24"
    initial_competition_numberOfPlaces = "25"
    club = {"name": "Club Example", "email": "club@example.com", "points": initial_club_points}
    competition = {"name": "Competition Example", "date": "2999-03-27 10:00:00", "numberOfPlaces": initial_competition_numberOfPlaces}
    ...
    assert club["points"] == str(int(initial_club_points) - int(places_to_book))
    mock_update_clubs.assert_called_once_with([club])
    print(club["points"])
    assert competition["numberOfPlaces"] == str(int(initial_competition_numberOfPlaces) - int(places_to_book))
